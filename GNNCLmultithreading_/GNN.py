...
#设置多核并行
#os.environ["OMP_NUM_THREADS"] = "300" # 1为一个核，设置为5的时候，系统显示用了10个核，不太清楚之间的具体数量关系
#tf.config.threading.set_intra_op_parallelism_threads(300)
#tf.config.threading.set_inter_op_parallelism_threads(300)
...
#os.system("pip install dgl dglgo")
#time.sleep(300)
...
#def getEmbedding(N,embed,args,g,i,net):
def getEmbedding(args,i,g,net,N):
    # 对大规模图进行采样
    ...
    if flag == False:
        res = [0]*(args.output_dim+1)
        print("第{}个节点图采样跳过! res={}  {}".format(i, res, datetime.datetime.now()))
    else:
        embed_tmp = net(g_sub, g_sub.ndata["feat"])
        j = tf.where(g_sub.ndata[dgl.NID] == i)[0, 0]  # 获取中心节点在原始图中真实的编号与当前子图中编号间的映射关系
        res = tf.concat([tf.constant([[i]], dtype=tf.float32), tf.reshape(embed_tmp[j, :], (1, -1))], axis=1).numpy().tolist()[0]
...
def test(args):
    ...
    #对子图进行取样
    embed = [[0]*(args.output_dim + 1)]
    #https://blog.csdn.net/lzy98/article/details/88819425
    with ThreadPoolExecutor() as executor:  # 实例化线程池，设置线程池的数量，不填则默认为cpu的个数*5
        future_tasks = [executor.submit(getEmbedding, args,i,g,net,N) for i in range(N)]
        executor.shutdown(True)
        for task in as_completed(future_tasks):
            if task.exception() == None:
                result = task.result()
                print("已完成第{}个任务的执行,时间为:{}".format(result[0],datetime.datetime.now()))
                if result != [0]*(args.output_dim+1):#此节点被采样子图
                    embed.append(result)
    '''for i in range(N):
        print("分发第{}个节点的任务 {}".format(i,datetime.datetime.now()))
        pool.apply_async(func=getEmbedding,args=(embed,args,i,))
    pool.close()
    pool.join()'''

    #转换为张量形式，避免修改execution.py的代码
    embed = tf.convert_to_tensor(embed,dtype=tf.float32)
